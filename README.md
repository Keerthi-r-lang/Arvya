...
2. Create `backend/.env` locally and add Razorpay **Test Mode** values for `RAZORPAY_KEY_ID` and `RAZORPAY_KEY_SECRET`. Never commit API secrets or place them in `.env.example`; without these values Arvya runs an explicitly labelled demo checkout.
