...
export default function App() {
  ...
  return <AppShell merchant={merchant} activePage={page} onNavigate={(target) => { setPage(target); setMessage(null); }} onLogout={logout} theme={theme} onThemeToggle={toggleTheme}>{page === "dashboard" ? <DashboardPage merchant={merchant} summary={summary} activity={activity} onOpenCatalog={() => setPage("catalog")} /> : <CatalogPage products={products} summary={summary} onUpload={handleUpload} onBack={() => setPage("dashboard")} message={message} />}</AppShell>;
}
