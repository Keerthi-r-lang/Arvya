...
def search_approved_offers(db: Session, query: str, budget_paise: int | None = None, merchant_id: int | None = None) -> dict:
    ...
    for recommendation, merchant in db.execute(statement):
        ...
        if items:
            ...
            product_names = list(db.scalars(select(Product.name).where(Product.id.in_(product_ids))))
        ...
        if budget_paise and final_price > budget_paise:
            continue
        searchable_text = " ".join([recommendation.title, recommendation.rationale, merchant.name, merchant.industry, *product_names])
        matched_terms = desired_terms.intersection(_terms(searchable_text))
        score = len(matched_terms) * 25 + int(recommendation.confidence_score * 25) + (10 if coupon else 0)
        if budget_paise:
    ...
    summary = "No approved offers match this search yet. Ask a merchant to generate and approve an offer first." if not results else f"Compared {len(results)} merchant-approved offers. The ranking uses intent relevance, offer confidence, coupon eligibility, and final price."
